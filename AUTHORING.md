...
- Tests: plain pytest functions, `@pytest.mark.parametrize`, deterministic (FakeClock, seeded `random.Random(42)`, no sleeps >50 ms), one concurrency test with `ThreadPoolExecutor` wherever a lock exists.
- **Name concurrency tests so they get preempted.** `code/conftest.py` drops the GIL switch interval to 1 us for any test whose name contains `concurrent`, `thread`, `race`, `parallel`, `simultaneous`, `atomic`, `lock`, `deadlock` or `contention`. At the default 5 ms a short critical section usually runs to completion, so a concurrency test passes even with its lock deleted — verified. If your test does not match one of those words, it is not actually testing the lock. LLD problems: ≥5 meaningful tests (happy path, validation error, state transition, concurrency, edge case). Patterns/HLD modules: ≥3.
