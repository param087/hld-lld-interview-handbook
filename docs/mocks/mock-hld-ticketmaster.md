...
> **Candidate:** So inventory is per-row, not a counter, which rules out the simplest design. Third: how long does a fan get to pay?
...
> **Candidate:** Worse than the writes, and I nearly skipped them. A million people on the queue page refreshing every five seconds is 200 thousand reads a second against about 200 on a normal day; one Redis node does around 100 thousand operations a second, so that alone needs replicas. The payload decides the rest: a 20 thousand-seat arena at one status byte per seat is a 20 kilobyte map, so 200 thousand readers is 4 gigabytes a second — 32 gigabits, untenable. The same readers taking 200-byte deltas is 40 megabytes a second, comfortable on one 10 gigabit link. A hundredfold difference from one choice.

> **Candidate:** Storage is an afterthought: 2 million orders a day at a kilobyte is 730 gigabytes a year, 2.2 terabytes at three replicas. And ten thousand live on-sales at 20 kilobytes each is 200 megabytes of packed maps, so the contended dataset fits in memory.
...
> **Candidate:** One connection pinned per fan for four minutes, with a row lock held throughout. At 2 thousand admitted fans a second that is hundreds of thousands of open transactions, so the pool is gone in the first second and every other query on the shard queues behind it. Not survivable — let me replace it.

> **Candidate:** The rule I should have started from: **a hold is data, not a lock.** One statement — `UPDATE seat SET status='held', hold_id=?, version=version+1 WHERE event_id=? AND seat_id IN (...) AND status='available' AND version=?` — committed immediately and accepted only if the row count equals the seats requested. The transaction lasts microseconds. The ten minutes of think-time become an `expires_at` column, which costs the database nothing. Two fans racing on 14C: one update touches the row, the other touches zero and gets a 409 naming 14C.

> **Interviewer:** Why not a Redis lock per seat with a TTL? Same effect, no database.

> **Candidate:** Because the lock and the truth would live in different systems: a failover that loses a key hands the same seat to two fans and the database never knows. Redis stays a cache and a queue, never the arbiter. A distributed lock needs fencing tokens, and at that point I have rebuilt the version column badly.

> **Candidate:** Two details go with it. Multi-seat holds are all-or-nothing, one statement over N rows, seat ids ordered deterministically so overlapping orders serialise instead of deadlocking. And expiry is lazy first, swept second: a seat past its TTL is takeable by the next reader, so a stalled sweeper degrades the map rather than freezing inventory.
...
> **Candidate:** By never asking the primary. The map is a **derived projection** no decision is made from. Every committed hold, release and confirm emits a `seat-changed` event of `(event_id, seat_id, status, version)`; a fan-out service applies those to a packed byte array per event in Redis, one status byte per seat, and bumps a map version. Clients fetch the array once, subscribe with the last version they saw, and replay from it on reconnect.

> **Candidate:** Server-sent events rather than WebSockets: one-directional traffic over ordinary HTTP, so proxies and CDNs work and reconnect with the last event id is built in. The justification is the earlier arithmetic — 4 gigabytes a second of full maps against 40 megabytes of deltas. For 200 thousand reads a second I add Redis replicas plus an in-process cache per seat-map server, refreshed from the event stream rather than by re-reading Redis.
...
- Reproduce from memory: the two-ratio estimation, the seat row with `status` and `version`, the three diagrams.

## Debrief

| Dimension | Below bar | Meets SDE2 | Exceeds |
|---|---|---|---|
| Requirements and scope | Sizes a fleet before asking whether overselling is allowed | Settles the invariant first — "Can we oversell?" — and derives strong consistency per partition from it | Reads the gift in an answer: a stale map means "the read path can be a cache and the write path the only authority" |
| Estimation | Reports 20 writes/s and concludes the system is easy | Puts both numbers side by side, derives the admission rate from a primary's capacity | Catches the read side unprompted: "4 gigabytes a second against 40 megabytes" |
| High-level design | Inventory reads and writes share one path; no queue anywhere | v1 by minute 18, waiting room by 30, cache and payments by 41 | Justifies the partition key by what it makes cheap: "a four-seat hold is a single-partition transaction" |
| Depth on the cruxes | One concurrency mechanism, no alternatives, no failure mode | Holds with TTL, the waiting room and the payment race, each with a number | Names all three payment-race outcomes before choosing, and ties reconciliation to them |
...
Hire at SDE2, noting that the locking turn cost two minutes that would have gone to cancellations and refunds, which never got drawn.

!!! warning "Common mistake"
    Holding a transaction open — or worse, a `SELECT ... FOR UPDATE` — for the ten minutes a fan spends entering card details. It passes every two-user demo and destroys the connection pool in the first second of a real on-sale, and it is the mistake that most often ends this round early. A hold is a row with an expiry; the only lock is the microsecond one inside the conditional update.

## Practice variants

Do each alone, on a clock, out loud.

1. **General admission only.** No seat map: three price tiers with capacities. Redo the concurrency deep dive as counters, then say what the waiting room is still for and what it is not. Fifteen minutes.
2. **The event is cancelled.** Fifty thousand bookings must be refunded and their seats returned, while three other on-sales run on the same platform. Design the refund path with idempotency keys, and say what the map shows during it. Fifteen minutes.
3. **Airbnb, not Marriott.** Inventory is a specific listing over a date range, not a pool, so two stays may not overlap on any night. What replaces the counter, where does atomicity live, and what changes for a host with 200 listings? Twenty minutes.

## Related

- [Design Ticketmaster (with a hotel-booking variant)](../hld/case-studies/ticketing-and-reservations.md) — the full design, hotel inventory table and seat lifecycle included
...
- [Latency numbers and estimation tables](../cheatsheets/latency-and-estimation.md) — the write rate, primary capacity and bandwidth figures from minutes 7 to 12
