...
Cutting the log into **segments** makes maintenance a file operation: retention deletes whole files, and a replica rolling back after an election truncates the tail. Each has a **sparse index**, one 8 B `(offset, byte position)` pair per 4 KB rather than per record, so a 1 GB segment indexes in ~2 MB and a lookup is a bisect plus a short scan.
