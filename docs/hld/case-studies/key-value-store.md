...
The write is acknowledged as soon as the WAL append is durable: one sequential disk write, no seek, which is what makes a replica write cost microseconds of CPU plus the fsync. The memtable is a sorted in-memory structure (a skip list in RocksDB) so the flush can stream entries in key order without sorting. Flushed SSTables are immutable, which is why they need no locks for readers and why they can be shipped whole when a node streams a key range to a new member. Reads check the memtable, then each SSTable's Bloom filter: at ~10 bits per key a filter says "definitely absent" for ~99% of the tables that do not hold the key, and the sparse index turns the remaining lookup into one block read (one SSD random read, ~16 µs). Compaction merges tables in the background, keeps the newest siblings per key, and drops tombstones older than the grace period; it is also the activity most likely to steal IO from reads, which is why coordinators wait for the fastest R replicas rather than a fixed set.

What to write on the whiteboard: `WAL -> memtable -> SSTable (Bloom + index) -> compaction`, then say "durability comes from the WAL, read speed from Bloom filters, and the cost is write amplification during compaction". The mechanics with measured amplification numbers are on the [storage engines page](../fundamentals/storage-engines-and-indexing.md); in the cluster module each node is a dict precisely so the replication logic stays readable.
