...
The gateway is the single entry point for cross-cutting policy — TLS termination, authentication, rate limiting, path routing — whose duties belong to [Load balancing, reverse proxies and API gateways](load-balancing-and-api-gateway.md). A backend-for-frontend is a gateway per client type, owned by that client's team: it composes a screen from several services, so a mobile home screen costs one round trip instead of five serial calls at ~150 ms, 750 ms before any work. Policy in the gateway, client-shaped composition in the BFF, business logic in neither.

### Sidecar and service mesh

A sidecar is a proxy (Envoy) beside every instance: the application talks to localhost and the sidecar does mTLS, discovery, retries, timeouts, circuit breaking, load balancing, metrics and trace propagation, identically in every language. A mesh (Istio, Linkerd) adds a control plane pushing routing rules, canary splits and policies to all sidecars. The price: two proxy hops per call, memory per pod, a platform to operate — worth it past a few dozen services in several languages, while five in one language get the same from a shared library.

### Sync vs async communication

A synchronous call (REST, gRPC) couples availability: five services at 99.9% in series are 0.999^5 ~ 99.5% — 0.005 x 8,760 h = ~44 hours of downtime a year instead of ~9 — with a latency floor of 5 x 0.5 ms = 2.5 ms before any work, so timeouts, retry budgets and circuit breakers from day one. An asynchronous message through a broker decouples in time: the producer learns "accepted", bursts become lag, new consumers subscribe unseen ([Messaging, queues and Kafka internals](messaging-and-event-streaming.md)). It costs eventual consistency, at-least-once duplicates (hence idempotent consumers) and harder debugging (hence correlation ids and tracing, see [Observability, SLOs and error budgets](observability-and-slos.md)). Rule: synchronous for a query this request needs answered (is the card valid), asynchronous for a state change others react to (order placed: notify, reserve, count).
...
Each service owns its schema; others get its data through its API or events, never its tables. Three consequences: no cross-service joins (denormalise from events, or compose in a BFF); no cross-service transactions (sagas and the transactional outbox, see [Transactions, 2PC, sagas and idempotency](transactions-and-distributed-transactions.md)); no ad-hoc reporting (a read model or a warehouse fed by events).

Skip the data split and you get the distributed monolith: services share a schema, a feature needs four pull requests deployed in order, every request crosses six synchronous hops — the network, the operations and the partial failures for none of the independence. The fix is rarely more services; it is merging back or redrawing data-ownership boundaries.

### CQRS

Command Query Responsibility Segregation separates the model that changes state from the models that answer queries. The write model is small, normalised and enforces invariants; each read model is denormalised for one query — a table per screen, a search index, a cache — and kept current from the write model's events, so reads lag by the projector's backlog: milliseconds when healthy. The motivation is asymmetry: reads outnumber writes 10:1 in a social product and 100:1 in a URL shortener, and want other indexes. The costs are two models to keep in sync and a read-your-writes gap. A CRUD admin screen gains nothing from either.
...
Event sourcing stores what happened to an aggregate — `AccountOpened`, `MoneyDeposited` — in an append-only stream and folds the events into state; the balance is a computation, not a column. Writes are optimistic, as the diagram above shows: replay the stream, validate the command, append events naming the version you expect, reload and retry if another writer got there first. The store needs a uniqueness constraint on `(stream_id, version)` and a global position for subscribers: a relational table does it, Kafka does not.

Snapshots bound the load cost: persist state every N events (100 in the demo) and a load is one snapshot plus the tail. Projections are CQRS read models fed by the global log, each with a checkpoint so redelivery is harmless, and a new question — every movement over 50.00 — is a new projector replayed from position 0: the audit trail, the temporal query and the bug fix in one mechanism. Size it: 10M accounts x 50 events a year x ~200 B is 100 GB a year, so a decade fits one server's 2-20 TB disk and a year replays from SSD at ~2 GB/s in ~50 s. The costs: events are immutable, so mistakes get compensating events, never edits; schemas are versioned and upcast; privacy deletion needs crypto-shredding; and the write side answers no ad-hoc queries.

### Strangler fig and serverless

Migrate a monolith behind a routing facade: move one capability at a time into a new service, shift its traffic — a percentage, then all — and delete the old copy. Data is the hard part: the new service must own its tables, so dual writes or change data capture until cutover. Serverless functions (Lambda, Cloud Functions) are the far end: a function per event, scaled to zero, billed per invocation, suiting bursty, short, stateless work — thumbnails, webhooks, glue — at the cost of cold starts, execution limits, per-invocation price at volume and connection-pool pressure on databases.
...
Start as a modular monolith and say so: one deployable, strict module boundaries, a schema per module, events on an in-process bus. That is one step from microservices — move the module whose team, load or blast radius demands it when the pressure is real. Choose microservices from day one only when the organisation already is one: several teams, separate on-call and release trains. When you split, split the data with the code, use synchronous calls for queries and events for state changes, and budget for the platform: discovery, gateway, timeouts, retries, tracing, common defaults. Apply CQRS where one aggregate is read in several shapes at volume — a product page, a timeline, a dashboard — and event sourcing where history is the product: ledgers, orders, inventory movements. For plain CRUD a table with an `updated_at` column is the right answer, and the interviewer knows it.

## Python implementation

`code/hld/event_sourcing.py` is a bank account stored as events — frozen dataclasses, with `StoredEvent` adding stream version and global position:

```python title="code/hld/event_sourcing.py — events"
--8<-- "code/hld/event_sourcing.py:events"
```

`BankAccount` keeps commands and state changes apart: `deposit`, `withdraw` and `close` validate the invariant and raise an event, `_apply` alone mutates state, so `replay` rebuilds the aggregate from a snapshot plus later events:

```python title="code/hld/event_sourcing.py — the aggregate"
--8<-- "code/hld/event_sourcing.py:aggregate"
```

`EventStore.append` checks the expected version and appends under one lock, making the concurrency optimistic rather than hopeful; `AccountRepository` turns that into load-and-save with snapshots:

```python title="code/hld/event_sourcing.py — store and repository"
--8<-- "code/hld/event_sourcing.py:store"
```

Projections consume the global log with a checkpoint, so redelivered events are skipped and `rebuild` replays from position 0 for a read model that did not yet exist:
...
Note the fourth line: the second writer loaded version 4, so its append is refused once the stream is at 5. The audit projection arrives last and still sees the first deposit.

## In the interview

Name the style with its reason as you draw the first box: "I'll start as a modular monolith with orders, payments and inventory as modules with their own schemas, and split payments out first for its compliance boundary and load profile."

Phrases that signal depth: "split on business capability, never on technical layer"; "database per service, so cross-service writes are sagas through an outbox"; "the read model is a projection, rebuildable from zero".

??? question "How do services find each other after a deploy changes every IP?"
    Through a registry fed by the orchestrator, not the instances. Server-side discovery costs one ~500 µs hop and keeps clients dumb; client-side saves the hop and costs a library per language.
...
??? question "Why not use Kafka as the event store?"
    Loading one aggregate means reading its events by key, and a topic can only be scanned by partition; compaction keeps the last value per key, destroying history. Kafka carries events to the projections; it is not the store.

??? question "A read model shows a stale balance right after a deposit. How do you handle it?"
    Return the new state from the command response, or route the writer's next read to the write model for a short window; alert on the projector's checkpoint falling behind.

??? question "Your team of six engineers wants twelve microservices. What do you say?"
    That we would ship a distributed monolith: twelve deploys, twelve on-call rotations and six-hop requests for one team's worth of change. Six modules in one deployable, split when a concrete pressure appears.

!!! tip "Interview tip"
    Volunteer the cost of every box you draw across a network: a 500 µs hop, a failure mode, an owner. Candidates who add services freely look junior; "this stays in the monolith until X" sounds like a veteran.

## Common mistakes

- **Splitting by technical layer**: a "user service" every request touches becomes a synchronous dependency of everything. Fix: split by business capability, each owning its data.
- **Shared database behind "microservices"**: schema changes couple deploys and the network buys nothing. Fix: database per service, with events or APIs for others.
- **Synchronous call chains**: five hops at 99.9% are 99.5% available with a 2.5 ms floor. Fix: events for state changes, timeouts and circuit breakers for the rest.
...
- **CQRS and event sourcing everywhere**: two models and a replay pipeline for an admin CRUD screen. Fix: reserve them for aggregates with many read shapes or an audit need.

!!! warning "Common mistake"
    Proposing microservices as the answer to "how would you scale this?" A monolith behind a load balancer scales to ~1k QPS per app server times as many servers as you like; microservices scale teams and blast radius, not throughput. Name the pressure, or do not split.

## Self-check

??? question "What does a modular monolith give you that a monolith does not, and what does it withhold?"
    Enforced ownership boundaries, a schema per module and a clean path to extraction; it withholds independent deployment and per-module scaling.
...
??? question "What does the expected version in an event-store append protect against?"
    Two writers that loaded the same state both appending: the second names a version the stream has passed and is refused, so no invariant is persisted from stale state.
...
??? question "Name three symptoms of a distributed monolith."
    A feature needs several repositories deployed in order; services share tables or a schema; most requests cross a long chain of synchronous calls, so one slow service degrades all.
