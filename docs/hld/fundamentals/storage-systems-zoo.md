...
Durability comes from redundancy, and the choice is replication or erasure coding. Three replicas cost 3x the bytes and survive two losses. Reed-Solomon RS(6,3) splits an object into 6 data and 3 parity chunks: 9/6 = 1.5x the bytes for the same three-failure tolerance, paid for with encode CPU and a reconstruct read whenever a chunk is missing. Erasure-code the cold bulk, replicate the small and hot: at 500k videos/day x 300 MB = 150 TB/day of ingest, that is 225 TB/day of disk instead of 450 TB/day.
...
??? question "Replication or erasure coding?"
    Replication for small, hot or latency-sensitive objects, where a read is one fetch. Erasure coding for cold bulk: RS(6,3) is 1.5x the bytes instead of 3x for the same three-failure tolerance, paid for with encode CPU and reconstruct reads.
