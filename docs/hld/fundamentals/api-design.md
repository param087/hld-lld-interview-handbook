...
Keyset (seek) pagination remembers where the last page ended: `WHERE (created_at, id) < (?, ?) ORDER BY created_at DESC, id DESC LIMIT 21`. With an index on `(created_at, id)` that is one seek and 21 rows whatever the page number, and an insert after the boundary cannot move what lies before it. The sort key must be total, which is why the id joins the timestamp: several orders in one millisecond would otherwise be skipped or repeated at a boundary. The extra row proves a next page exists without a `COUNT(*)`.
...
Filters and sorts are query parameters over indexed columns (`?customer_id=42&status=shipped&sort=-created_at`): allow sorting only on fields indexed together with the tie-break id, since a keyset page on an unindexed sort is a full scan, and reject unknown fields with a 400 that names them. Both belong in the cursor's fingerprint, so a client cannot change the filter halfway through a walk.
...
Those limits, like TLS termination, authentication, size and timeout caps, CORS and `/v1` versus `/v2` routing, are gateway policies rather than service code — [Load balancing, reverse proxies and API gateways](load-balancing-and-api-gateway.md) has the duty list and prices the hop.

### Long-running operations: 202 and poll, or webhook

Work that outlives a client's patience — a transcode, a report, a bulk import — must not hold the connection. Return `202 Accepted` with an operation resource, `GET /v1/operations/{id}`, carrying a status (pending, running, succeeded, failed), progress and, on success, the result's location. The client polls at the interval you suggest in `Retry-After`, and the operation document is cheap to serve from cache. Polling costs empty reads — 100k pending operations polled every 5 s is 20k QPS of "still running" — so offer a webhook too: browsers and phones poll, partners subscribe.
...
A webhook is your server calling theirs, so every client failure mode is now yours. Deliver at least once: retry with exponential backoff and jitter for hours, then park the event in a dead-letter list the customer can replay. Because delivery is at least once, every event carries a unique id and the contract states that receivers deduplicate. Sign every payload — HMAC-SHA256 over timestamp and body with a per-endpoint secret, verified in constant time with a five-minute tolerance against replay. Never promise ordering across retries: carry a sequence number or resource version so receivers drop stale events, or send thin events that say to fetch current state.

### Batch endpoints

When a client needs 100 orders by id, 100 GETs cost 100 round trips: 100 x 500 µs = 50 ms inside a datacenter, 100 x 70 ms = 7 s from a phone across the US. Offer `GET /v1/orders?ids=1,2,...` capped at 100 ids, or `POST /v1/orders:batchGet` when the list outgrows a URL. Batch writes return per-item results, because partial failure is normal and a single 400 for the whole batch forces the client to retry everything. Bound every batch and make batch writes idempotent per item.
...
Default to REST with JSON at the edge: every client, proxy, CDN and debugger already speaks it, GET responses cache by URL, and the conventions above make the contract predictable for people who have never read your docs. Move to gRPC between services once payload size, parse cost and a typed `.proto` contract pay for themselves, keeping a REST or gRPC-Web gateway for anything a browser calls. Reach for GraphQL when many client screens want different shapes of the same data and the mobile team is paying in round trips — 150 ms each between California and the Netherlands — and accept that you now own a query planner, per-request batching loaders, cost limits and a cache that no longer keys by URL. [Networking for system design](networking-essentials.md) prices all three on the wire, with protobuf's field-number evolution rules and the N+1 fan-out in detail. Whatever the style, the asynchronous half of the contract is the same: a 202 with an operation to poll, signed at-least-once webhooks for partners, and idempotency keys wherever a client might retry. Pick one style per boundary and say why; three styles in one service tier is the mark of a system nobody designed.
...
Write the endpoints as a table in minute nine and state the conventions once: "Resources are plural nouns under `/v1`; every write a client might retry takes an `Idempotency-Key`; every list returns an opaque keyset `next_cursor`; errors share one envelope with a machine-readable code; the gateway answers 429 with `Retry-After`." That sentence answers most follow-ups before they are asked, and the table carries the rest of the round.
...
??? question "How does the server tell a legitimate retry from a duplicate order?"
    By the `Idempotency-Key`: same key and body replays the stored response, same key with a different body is a client bug (422), a new key is a new order. Without the header there is no way to tell, which is why it is mandatory on POST.

??? question "A webhook receiver was down for an hour. What do they see when it comes back?"
    Your retries with backoff, possibly out of order and duplicated, each carrying an event id and a signature. They deduplicate by id, verify the signature inside the timestamp tolerance, and apply by version. Events that exhausted their retries wait in a dead-letter list.
...
!!! tip "Interview tip"
    Put `Idempotency-Key` and `cursor` in the endpoint table before anyone asks. Interviewers read the API step in seconds, and those two words say you have handled retries and pagination in production; their absence is the first "what if" they reach for.
