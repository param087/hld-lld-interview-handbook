...
A third family exists; name it only if asked. `Catalog._meta_lock` and `InMemoryShowRepository._lock` guard read-mostly catalog dicts, and exactly one cross-family nesting exists in the package: `cancel` looks the show up while holding `_bookings_lock`, never the reverse. A seat lock is never held while either of the other two is.
